                ...
                out.append(
                    {
                        ...
                        "created_at": int(r["created_at"]) if r.get("created_at") else None,
                        "updated_at": int(r["updated_at"]) if r.get("updated_at") else None,
                    }
                )
            return out
...
def api_stats():
    ...
    try:
        ...
        if targets:
            updated_at = max((t.get("updated_at") or t.get("created_at") or "") for t in targets)
    except Exception:
        updated_at = None
...
def upsert_launch_site(s: LaunchSiteIn):
    ...
    if not found:
        ...
        items.append(found)
    found["lat"] = float(s.lat) if s.lat is not None else None
    found["lng"] = float(s.lng) if s.lng is not None else None
    found["active"] = bool(s.active)
    _save_launch_sites(items)
...
def update_target(target_id: str, t: TargetIn):
    ...
    found["speed_kmh"] = float(t.speed_kmh or 0)
    found["dest_lat"] = float(t.dest_lat) if t.dest_lat is not None else None
    found["dest_lng"] = float(t.dest_lng) if t.dest_lng is not None else None
