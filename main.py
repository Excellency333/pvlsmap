...
def _db_init() -> None:
    ...
    with _db_conn() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("alter table pvls_targets add column if not exists active boolean default true;")
...
def _load_targets() -> list[dict]:
    try:
        ...
        with open(DATA_PATH, "r", encoding="utf-8") as f:
            ...
            # backward compatible: add missing fields
            for it in items:
                if "active" not in it:
                    it["active"] = True
            return items
...
# For security, set ADMIN_PASSWORD_HASH and ADMIN_SALT in Render env.
# Fallback to plain default password ONLY if no hash is provided.
ADMIN_PASSWORD_PLAIN = os.getenv("ADMIN_PASSWORD", "12344321")
ADMIN_SALT = os.getenv("ADMIN_SALT", "pvls_default_salt_change_me")
ADMIN_PASSWORD_HASH = os.getenv("ADMIN_PASSWORD_HASH", "")

ADMIN_SECRET = os.getenv("ADMIN_SECRET", "") or os.getenv("SECRET_KEY", "") or "pvls_secret_change_me"

def _pw_hash(pw: str) -> str:
    dk = hashlib.pbkdf2_hmac("sha256", pw.encode("utf-8"), ADMIN_SALT.encode("utf-8"), 120_000)
    return dk.hex()

def _check_pw(pw: str) -> bool:
    if ADMIN_PASSWORD_HASH:
        return hmac.compare_digest(_pw_hash(pw), ADMIN_PASSWORD_HASH)
    return hmac.compare_digest(pw, ADMIN_PASSWORD_PLAIN)
...
def _verify_token(token: str) -> Optional[str]:
    try:
        ...
        return None
    exp_sig = hmac.new(ADMIN_SECRET.encode("utf-8"), payload.encode("utf-8"), hashlib.sha256).hexdigest()
...
def require_admin(req: Request) -> None:
    tok = req.cookies.get("pvls_admin", "")
    user = _verify_token(tok) if tok else None
...
def admin_login_page():
    return HTMLResponse((APP_DIR / "templates" / "admin_login.html").read_text(encoding="utf-8"))

@app.post("/admin/login")
def admin_login(data: LoginIn):
    if data.username != ADMIN_USER or not _check_pw(data.password):
        raise HTTPException(status_code=401, detail="Bad credentials")
    exp = int(time.time()) + (30*24*3600 if data.remember else 8*3600)
    payload = f"{data.username}|{exp}"
    token = _sign_token(payload)
    resp = JSONResponse({"ok": True})
    max_age = 30*24*3600 if data.remember else 8*3600
    resp.set_cookie(
        "pvls_admin",
        token,
        max_age=max_age,
...
def admin_logout():
    resp = JSONResponse({"ok": True})
    resp.delete_cookie("pvls_admin", path="/")
    return resp
@app.get("/api/targets")
def get_targets():
...
def add_target(t: TargetIn, _auth=Depends(require_admin_dep)):
...
def delete_target(target_id: str, _auth=Depends(require_admin_dep)):
    if os.getenv("DATABASE_URL") and psycopg2 is not None:
        _db_delete_target(target_id)
        return JSONResponse({"ok": True})
@app.delete("/api/targets/clear")
def clear_targets(_auth=Depends(require_admin_dep)):
    if os.getenv("DATABASE_URL") and psycopg2 is not None:
        _db_init()
        with _db_conn() as conn:
            with conn.cursor() as cur:
...
def update_target(target_id: str, t: TargetIn, _auth=Depends(require_admin_dep)):
    item = {
        "id": target_id,
        "type": t.type,
        "lat": float(t.lat),
