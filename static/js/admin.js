document.addEventListener("DOMContentLoaded", ()=>{
  ...
  // Custom Ukrainian labels only for the Pavlograd district area.
  // This helps when the basemap falls back to English or small labels are too light.
  function createPvlsPlaceLabelPane(){
    if(map.getPane('pvlsPlaceLabels')) return map.getPane('pvlsPlaceLabels');
    const pane = map.createPane('pvlsPlaceLabels');
    pane.style.zIndex = 560;
    pane.style.pointerEvents = 'none';
    return pane;
  }
  createPvlsPlaceLabelPane();

  const pvlsPlaceDefs = [
    {name:'ПАВЛОГРАДСЬКИЙ РАЙОН', lat:48.664, lng:35.983, kind:'district', minZoom:8},
    {name:'Павлоград', lat:48.535, lng:35.870, kind:'city', minZoom:8},
    {name:'Тернівка', lat:48.523, lng:36.092, kind:'town', minZoom:9},
    {name:'Петропавлівка', lat:48.456, lng:36.438, kind:'town', minZoom:9},
    {name:'Юр’ївка', lat:48.816, lng:35.918, kind:'town', minZoom:9},
    {name:'Шахтарське', lat:48.390, lng:36.200, kind:'town', minZoom:10},
    {name:'Богданівка', lat:48.485, lng:36.020, kind:'village', minZoom:10},
    {name:'Богуслав', lat:48.448, lng:35.710, kind:'village', minZoom:10},
    {name:'Самарське', lat:48.420, lng:35.975, kind:'village', minZoom:10},
    {name:'Вербки', lat:48.615, lng:36.050, kind:'village', minZoom:10},
    {name:'Межиріч', lat:48.592, lng:35.758, kind:'village', minZoom:10},
    {name:'Дмитрівка', lat:48.309, lng:36.221, kind:'village', minZoom:10},
  ];
  const pvlsPlaceLayer = L.layerGroup();
  const pvlsPlaceMarkers = pvlsPlaceDefs.map((p)=>({
    def:p,
    marker:L.marker([p.lat,p.lng], {
      pane:'pvlsPlaceLabels',
      interactive:false,
      keyboard:false,
      zIndexOffset:-200,
      icon:L.divIcon({
        className:'pvls-place-label-icon',
        html:`<div class="pvls-place-label ${p.kind}">${p.name}</div>`,
        iconSize:null,
        iconAnchor:[0,0]
      })
    })
  }));
  function updatePvlsPlaceLabels(){
    const enabled = !!baseMapTiler && currentBase === baseMapTiler;
    const z = map.getZoom();
    for(const item of pvlsPlaceMarkers){
      const shouldShow = enabled && z >= (item.def.minZoom || 0);
      const has = pvlsPlaceLayer.hasLayer(item.marker);
      if(shouldShow && !has) pvlsPlaceLayer.addLayer(item.marker);
      else if(!shouldShow && has) pvlsPlaceLayer.removeLayer(item.marker);
    }
    if(enabled){
      if(!map.hasLayer(pvlsPlaceLayer)) pvlsPlaceLayer.addTo(map);
    }else if(map.hasLayer(pvlsPlaceLayer)){
      map.removeLayer(pvlsPlaceLayer);
    }
  }
  ...
  map.on('zoomend', updatePvlsPlaceLabels);
  ...
  function currentRadiusM(){
    const km=Math.max(1, Math.min(50, parseFloat(zoneRadius?.value||'10')||10));
    return km*1000;
  }

  function updateZoneRadiusLabel(){
    if(zoneRadiusVal) zoneRadiusVal.textContent=`${parseFloat(zoneRadius?.value||'10')||10} км`;
  }
  ...
  function selectZone(id){
    ...
    for(const [zid,o] of zoneObjects.entries()){
      ...
      try{o.circle.setStyle({weight:selected?4:2, opacity:selected?0.95:0.78, fillOpacity:selected?0.18:0.13});}catch(_){ }
    }
    const z=threatZones.find(x=>x.id===selectedZoneId);
    if(z && zoneRadius){
      zoneRadius.value=String(Math.max(1, Math.min(50, Math.round(Number(z.radius_m||10000)/1000))));
    }
    updateZoneRadiusLabel();
  ...
  async function persistZone(z){
    if(!z) return null;
    const saved=await apiPost(`/api/zones/${z.id}`, {
      lat:Number(z.lat), lng:Number(z.lng), radius_m:Number(z.radius_m)
    });
    const idx=threatZones.findIndex(x=>x.id===saved.id);
    if(idx>=0) threatZones[idx]=saved;
    upsertZone(saved);
    return saved;
  }
  ...
  function upsertZone(z){
    ...
    if(zoneObjects.has(z.id)){
      ...
      const center=L.marker(ll,{icon:zoneCenterIcon(),draggable:true,zIndexOffset:800}).addTo(map);
      const pick=()=>selectZone(z.id);
      circle.on('click',(e)=>{try{L.DomEvent.stopPropagation(e);}catch(_){ } pick();});
      ...
      zoneObjects.set(z.id,{circle,center,z});
    }
  ...
  function renderZoneList(){
    ...
    threatZones.forEach((z,idx)=>{
      ...
      row.innerHTML=`<div class="meta"><div class="t">🔴 Зона ${idx+1}</div><div class="s">Радіус ${(Number(z.radius_m||0)/1000).toFixed(Number(z.radius_m||0)%1000?1:0)} км • ${Number(z.lat).toFixed(4)}, ${Number(z.lng).toFixed(4)}</div></div><div class="actions"><button class="btn small" data-act="select">Вибрати</button><button class="btn small danger" data-act="del">Видалити</button></div>`;
  ...
  if(zoneRadius){
    ...
    zoneRadius.addEventListener('change',async()=>{
      if(!selectedZoneId) return;
      const o=zoneObjects.get(selectedZoneId);
      if(!o) return;
      o.z.radius_m=currentRadiusM();
  ...
  document.querySelectorAll('.zone-radius-preset').forEach(btn=>{
    btn.addEventListener('click',()=>{
      ...
      zoneRadius.value=String(btn.dataset.km||'10');
  ...
  if(zoneDelete){
    zoneDelete.addEventListener('click',async()=>{
      if(!selectedZoneId) return;
      await apiDelete(`/api/zones/${selectedZoneId}`);
      selectedZoneId=null;
      await loadZones();
      toast('Зону видалено');
    });
