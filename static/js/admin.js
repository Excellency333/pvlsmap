document.addEventListener("DOMContentLoaded", ()=>{
  ...
  const btnPickDest=document.getElementById("pickDest");
  const btnClearDest=document.getElementById("clearDest");
  let pickingDest=false;
  let destMarker=null;
  let destLine=null;

  function isBallistic(){ return (elType && elType.value==="ballistic"); }
  function showDest(show){
    if(!destRow) return;
    destRow.style.display = show ? "block" : "none";
  }
  function updateDestPreview(){
    try{
      if(!isBallistic()) { if(destMarker){ map.removeLayer(destMarker); destMarker=null; } if(destLine){ map.removeLayer(destLine); destLine=null; } return; }
      const ll=latLngFromInputs();
      const dlat=parseFloat(elDestLat?.value||"");
      const dlng=parseFloat(elDestLng?.value||"");
      if(!ll || isNaN(dlat) || isNaN(dlng)) { if(destMarker){ map.removeLayer(destMarker); destMarker=null; } if(destLine){ map.removeLayer(destLine); destLine=null; } return; }
      const d=L.latLng(dlat,dlng);
      if(!destMarker){ destMarker=L.marker(d,{draggable:true,opacity:0.95}).addTo(map);
        destMarker.on("drag",(e)=>{ const p=e.target.getLatLng(); if(elDestLat) elDestLat.value=p.lat.toFixed(5); if(elDestLng) elDestLng.value=p.lng.toFixed(5); if(destLine) destLine.setLatLngs([ll,p]); });
        destMarker.on("dragend", async ()=>{ updateDestPreview(); try{ if(selectedId) await saveTarget(); }catch(_){} });
      } else { destMarker.setLatLng(d); }
      if(!destLine){ destLine=L.polyline([ll,d],{weight:4,opacity:0.9,color:"#ff3b5b",dashArray:"6 8"}).addTo(map);} else { destLine.setLatLngs([ll,d]); }
    }catch(_){ }
  }
  ...
  async function saveTarget(){
    ...
    if(isBallistic()){ payload.dest_lat = (elDestLat && elDestLat.value? parseFloat(elDestLat.value): null); payload.dest_lng = (elDestLng && elDestLng.value? parseFloat(elDestLng.value): null); }
    await apiPost(`/api/targets/${selectedId}`, payload);
  }

  map.on("click",(e)=>{
    // pick destination point for ballistic
    if(pickingDest && isBallistic()){
      pickingDest=false;
      if(elDestLat) elDestLat.value=e.latlng.lat.toFixed(5);
      if(elDestLng) elDestLng.value=e.latlng.lng.toFixed(5);
      showDest(true);
      updateDestPreview();
      toast("Кінцеву точку збережено");
      // auto-save if selected
      if(selectedId){ saveTarget().catch(()=>{}); }
      return;
    }
  ...
  if(btnPickDest){
    btnPickDest.addEventListener("click",(e)=>{
      ...
      showDest(true);
      pickingDest=true;
      toast("Ткни на мапі: вибери кінцеву точку");
    });
  }
  if(btnClearDest){
  ...
  initTypeSelect();
  elType.addEventListener("change", ()=>{ showDest(isBallistic()); updateDestPreview(); });
  reload();
  loadLaunch();
});