document.addEventListener("DOMContentLoaded", ()=>{
  ...
  let targetBridgeRaf=0;
  let targetBridgePending=[];
  function bridgeTargetsFrame(targets){
    targetBridgePending=Array.isArray(targets)?targets.filter(Boolean):[];
    if(targetBridgeRaf) return;
    targetBridgeRaf=requestAnimationFrame(()=>{
      targetBridgeRaf=0;
      const list=targetBridgePending;
      targetBridgePending=[];
      if(!list.length) return;
      if(list.length===1) bridgeTarget("upsert",{target:list[0]});
      else bridgeTarget("bulk_upsert",{targets:list});
    });
  }
  function liveTargetSnapshot(id, latlng=null){
    const o=liveMarkers.get(id);
    if(!o) return null;
    const t=o.t||{};
    const p=latlng || o.m.getLatLng();
    return {
      ...t, id, lat:p.lat, lng:p.lng,
  ...
  function selectCruisesInBounds(bounds){
    ...
    if(n>0){
      ...
      toast(`Виділено крилатих: ${n}. Перетягування рухає групу, курс зараз змінює всю групу.`);
    }else{
  ...
  function previewGroupDrag(anchorId, anchorLatLng){
    ...
    for(const [id,start] of groupDragState.starts.entries()){
      ...
      if(id===anchorId){
        ...
        ll=map.unproject(p,groupDragState.zoom);
        try{o.m.setLatLng(ll);}catch(_){ }
      }
      o.t={...o.t,lat:ll.lat,lng:ll.lng};
      try{o.l.setLatLngs(arrowPolyline(ll,parseInt(o.t.direction||0,10)||0));}catch(_){ }
      const snap=liveTargetSnapshot(id,ll);
      if(snap) live.push(snap);
    }
    bridgeTargetsFrame(live);
  }
  ...
  async function saveGroupDrag(anchorId){
    ...
    for(const id of ids){
      ...
      o.t={...o.t,lat:p.lat,lng:p.lng};
      const payload=targetPayloadFromLive(o);
      if(!payload) continue;
      jobs.push({id,target:payload});
      const snap=liveTargetSnapshot(id,p);
      if(snap) optimistic.push(snap);
    }
    if(!jobs.length) return true;

    // Viewer moves immediately; server persistence follows in one bulk UPDATE-only request.
    bridgeTarget("bulk_upsert",{targets:optimistic});
    try{
      const response=await apiPost('/api/targets/bulk',{items:jobs});
      const results=(response && Array.isArray(response.targets)) ? response.targets : [];
      ...
      if(results.length) bridgeTarget("bulk_upsert",{targets:results});
      refreshGroupSelectionStyles();
      ...
      try{lastAdminUpdated=null; await reload();}catch(_){ }
      return true;
    }
  }
  ...
  function saveGroupDirections(){
    ...
    groupDirectionSaveQueue = groupDirectionSaveQueue.catch(()=>{}).then(async()=>{
      const response=await apiPost('/api/targets/bulk',{items:jobs.map(j=>({id:j.id,target:j.payload}))});
      const results=(response && Array.isArray(response.targets)) ? response.targets : [];
      // Don't let an older save response repaint a newer local course.
      if(revision===groupDirectionSaveRevision){
        bridgeTarget("bulk_upsert",{targets:results});
        for(const saved of results){
          if(!saved || !saved.id) continue;
          const o=liveMarkers.get(saved.id);
          if(o){ o.t={...o.t,...saved}; }
    ...
    if(liveMarkers.has(t.id)){
      ...
      m.on("drag", (e)=>{
        ...
        if(groupDragState && groupDragState.anchorId===t.id){
          ...
          const cur=liveMarkers.get(t.id);
          if(cur){
            cur.t={...cur.t,lat:p.lat,lng:p.lng};
            const snap=liveTargetSnapshot(t.id,p);
            if(snap) bridgeTargetsFrame([snap]);
          }
        }
      ...
      m.on("dragend", async (e)=>{
        ...
        try{
          ...
          if(saved){
            bridgeTarget("upsert",{target:saved});
            upsertMarker(saved);
            fillForm(saved);
          }
        }catch(_){ }
  ...
  function targetFromForm(id){
    const ll=latLngFromInputs();
    if(!ll) return null;
    return {
      id, type:elType.value, lat:ll.lat, lng:ll.lng,
  ...
  document.getElementById("save").addEventListener("click", async ()=>{
    ...
    const tempId=wasNew ? `tmp_${Date.now()}_${Math.random().toString(36).slice(2,7)}` : originalId;
    const optimistic=targetFromForm(tempId);
    if(!optimistic){ alert("Ткни на мапі або введи координати"); return; }

    // Show it in admin/viewer immediately, before network round-trip.
    upsertMarker(optimistic);
    bridgeTarget("upsert",{target:optimistic});
    try{
      // Keep selectedId semantics intact: new targets still call POST /api/targets.
      selectedId=originalId;
      const saved=await saveTarget();
      if(!saved) throw new Error('empty save response');
      if(wasNew){
        removeTargetUI(tempId);
        bridgeTarget("delete",{id:tempId});
      }
      bridgeTarget("upsert",{target:saved});
      upsertMarker(saved);
      ...
      console.error(err);
      if(wasNew){ removeTargetUI(tempId); bridgeTarget("delete",{id:tempId}); }
      else{ try{lastAdminUpdated=null; await reload();}catch(_){ } }
      toast('Не вдалося зберегти ціль');
    }
  });
