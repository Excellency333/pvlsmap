document.addEventListener("DOMContentLoaded", ()=>{
  ...
  // Optional MapTiler vector basemap. It is deliberately isolated so a CDN/API
  // failure never breaks the existing OSM/CARTO layers.
  function createPvlsMapTilerLayer(){
    try{
      const key = String(window.PVLS_MAPTILER_KEY || "").trim();
      if(!key || !L.maptiler || typeof L.maptiler.maptilerLayer !== "function") return null;
      const style = "bright-v2";
      const language = "uk";
      ...
      layer.on("ready", ()=> enhancePvlsMapTilerLabels(layer));
      return layer;
    }catch(err){
      console.warn("MapTiler layer unavailable; keeping standard basemaps", err);
      return null;
    }
  }
