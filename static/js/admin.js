document.addEventListener("DOMContentLoaded", ()=>{
  ...
  const layerCtl = L.control.layers({"Схема": baseOSM, "Темна": baseDark}, {}, {collapsed:true}); // layers control removed (single theme button)
  ...
  function drawBallisticLine(){
    ...
    ballisticLine=L.polyline([start,end],{weight:4,opacity:0.9,color:"#ff3b5b"}); // layers control removed (single theme button)
    if(ballisticDestMarker){ map.removeLayer(ballisticDestMarker); }
    ballisticDestMarker=L.circleMarker(end,{radius:7,weight:2,opacity:0.9,fillOpacity:0.35,color:"#ff3b5b"}); // layers control removed (single theme button)
  ...
  function fillForm(t){
    ...
    elNote.value=t.note||"";
    if(elActive){ elActive.checked = (t.active === undefined) ? true : !!t.active; }
  ...
  function upsertMarker(t){
    ...
    if(liveMarkers.has(t.id)){
      ...
      const m=L.marker(ll,{icon,rotationAngle:ang,rotationOrigin:'center center',draggable:true}); // layers control removed (single theme button)
      const l=L.polyline(arrowPolyline(ll,ang),{weight:3,opacity:0.75}); // layers control removed (single theme button)
  ...
  if(btnDeleteAll){
    btnDeleteAll.addEventListener("click", async ()=>{
      ...
      await apiDelete("/api/targets/clear");
      await reload(); clearForm();
    });
  }
