document.addEventListener("DOMContentLoaded", ()=>{
  ...
  // Native MapTiler Hybrid satellite layer with MapTiler's own Ukrainian labels.
  // No manual city labels or Esri overlay: the entire cartographic style handles
  // label placement, scale visibility, roads and borders consistently.
  function createPvlsSatelliteUaLayer(){
    const key=String(window.PVLS_MAPTILER_KEY||'').trim();
    if(!key) return null;
    try{
      const ns = (L.maptiler || window.leafletmaptilersdk || {});
      const factory =
        (L.maptiler && typeof L.maptiler.maptilerLayer==='function' && L.maptiler.maptilerLayer.bind(L.maptiler)) ||
        (window.leafletmaptilersdk && typeof window.leafletmaptilersdk.maptilerLayer==='function' && window.leafletmaptilersdk.maptilerLayer.bind(window.leafletmaptilersdk)) ||
        (typeof L.maptilerLayer==='function' && L.maptilerLayer.bind(L));
      ...
      const hybridStyle =
        (ns.MapStyle && ns.MapStyle.HYBRID) ||
        (window.maptilersdk && window.maptilersdk.MapStyle && window.maptilersdk.MapStyle.HYBRID) ||
        'hybrid';
      const ukLanguage =
        (ns.Language && ns.Language.UKRAINIAN) ||
        (ns.MaptilerLanguage && ns.MaptilerLanguage.UKRAINIAN) ||
      ...
      if(layer && typeof layer.on==='function'){
        layer.on('ready',()=>{
          try{ if(typeof layer.setLanguage==='function') layer.setLanguage(ukLanguage); }catch(_){ }
        });
      }
      return layer || null;
    }catch(err){
      console.warn('MapTiler Hybrid UA unavailable',err);
      return null;
    }
  }
