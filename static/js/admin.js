document.addEventListener("DOMContentLoaded", ()=>{
  ...
  function forcePvlsUkrainianPlaceNames(layer){
    try{
      const mt = layer && layer.getMaptilerMap ? layer.getMaptilerMap() : null;
      if(!mt) return;
      const uk =
      ...
      for(const sl of layers){
        if(!sl || sl.type !== "symbol" || !sl.layout || sl.layout["text-field"] == null) continue;
        const signature = `${String(sl.id||"")} ${String(sl["source-layer"]||"")}`;
        const placeLabel = /(place|settlement|city|town|village|hamlet|locality|country|state|region|province|county|municip|neighbou?rhood)/i.test(signature);
        if(!placeLabel) continue;
        // Prefer the real Ukrainian OSM name. If it is missing, use the local name
        // before any Latin/English fallback. This keeps Ukrainian places Cyrillic.
        const ukName = [
          "coalesce",
          ["get","name:uk"],
          ["get","name_uk"],
        ...
        try{ mt.setLayoutProperty(sl.id, "text-field", ukName); }catch(_){ }
      }
    }catch(err){
      console.warn("MapTiler Ukrainian label enforcement skipped", err);
    }
  }
  ...
  function createPvlsHybridUaLayer(){
    ...
    if(!key) return null;
    try{
      const factory = L.maptiler && typeof L.maptiler.maptilerLayer === "function"
        ? L.maptiler.maptilerLayer.bind(L.maptiler)
        : null;
      if(factory){
        const style =
          (L.maptiler.MapStyle && L.maptiler.MapStyle.HYBRID) ||
          (window.maptilersdk && window.maptilersdk.MapStyle && window.maptilersdk.MapStyle.HYBRID) ||
          "hybrid-v4";
        const language =
          (L.maptiler.Language && L.maptiler.Language.UKRAINIAN) ||
          (window.maptilersdk && window.maptilersdk.Language && window.maptilersdk.Language.UKRAINIAN) ||
          "uk";
        const layer = factory({
          apiKey:key,
        ...
        if(layer && typeof layer.on === "function"){
          layer.on("ready", ()=>{
            forcePvlsUkrainianPlaceNames(layer);
            // Some styles finish replacing symbol layers shortly after ready.
            setTimeout(()=>forcePvlsUkrainianPlaceNames(layer), 250);
            setTimeout(()=>forcePvlsUkrainianPlaceNames(layer), 900);
          });
        }
        if(layer) return layer;
      }
    }catch(err){
      console.warn("MapTiler Hybrid vector layer unavailable; using label-free satellite fallback", err);
    }

    // Never fall back to the raster Hybrid because its Latin labels are baked in.
    // A plain satellite fallback is preferable: no wrong-language labels.
    return L.tileLayer(`https://api.maptiler.com/maps/satellite-v4/{z}/{x}/{y}.jpg?key=${encodeURIComponent(key)}`, {
      tileSize:512,
      zoomOffset:-1,
      minZoom:1,
      maxZoom:20,
