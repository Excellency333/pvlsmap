...
const dir = (Number.isFinite(dirNum) ? dirNum : 0);
  ...
  function upsertTargetInstant(t){
    if(!t) return;
    const id=String(t.id ?? t._id ?? t.key ?? "");
    if(!id) return;
    const isNew=!markers.has(id);
    // Temporary optimistic IDs should not create duplicate feed events.
    upsert(t,isNew && !id.startsWith("tmp_"));
  ...
  try{
    if("BroadcastChannel" in window){
      ...
      pvlsLiveBridge.addEventListener("message",(ev)=>{
        ...
        if(d.action==="upsert" && d.target) upsertTargetInstant(d.target);
        else if(d.action==="bulk_upsert" && Array.isArray(d.targets)){
          for(const t of d.targets) upsertTargetInstant(t);
        }
        else if(d.action==="delete") removeTargetInstant(d.id);
  ...
  function applyTargetPush(data){
    ...
    if(p.action==="upsert" && p.target){
      upsertTargetInstant(p.target);
      const u=document.getElementById("updated");
      if(u && data.updated_at) u.textContent=`Оновлено: ${formatTs(data.updated_at)}`;
      return true;
    }
    if(p.action==="bulk_upsert" && Array.isArray(p.targets)){
      for(const t of p.targets) upsertTargetInstant(t);
      const u=document.getElementById("updated");
      if(u && data.updated_at) u.textContent=`Оновлено: ${formatTs(data.updated_at)}`;
      return true;
    }
