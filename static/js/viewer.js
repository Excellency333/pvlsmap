document.addEventListener("DOMContentLoaded", ()=>{
  ...
  // Stable raster basemaps. The old CARTO/CyclOSM endpoints became unreliable,
  // so these equivalents use the already configured MapTiler key.
  const mapTilerKey = String(window.PVLS_MAPTILER_KEY || '').trim();
  function mapTilerRaster(styleId){
    return L.tileLayer(`https://api.maptiler.com/maps/${styleId}/256/{z}/{x}/{y}.png?key=${encodeURIComponent(mapTilerKey)}`, {
      maxZoom:20, crossOrigin:true, attribution:'&copy; MapTiler &copy; OpenStreetMap contributors'
    });
  }
  const baseVoyager = mapTilerRaster('streets-v4');
  const baseCyclOSM = mapTilerRaster('outdoor-v4');
  ...
  const baseDark = mapTilerRaster('dataviz-v4-dark');

  // Detailed aerial/satellite mosaic (older in some places, but sharp).
  const baseSatellite = L.tileLayer(
    'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',
    {maxZoom:20, maxNativeZoom:19, attribution:'Tiles &copy; Esri — World Imagery'}
  );

  // Fresh Sentinel-2 true-colour imagery from Terrascope (public WMS, 10 m source data).
  // Request a rolling recent window so the layer keeps updating without code changes.
  function sentinel2TimeWindow(){
    const to=new Date();
    const from=new Date(to.getTime()-21*24*60*60*1000);
    const d=(x)=>x.toISOString().slice(0,10);
    return `${d(from)}/${d(to)}`;
  }
  const baseSentinelFresh = L.tileLayer.wms('https://titiler.terrascope.be/wms', {
    layers:'terrascope-s2-toc-v2_tci',
    format:'image/png',
    transparent:false,
    version:'1.3.0',
    time:sentinel2TimeWindow(),
    maxZoom:20,
    maxNativeZoom:14,
    attribution:'Sentinel-2 / Copernicus • Terrascope'
  });
  ...
  const baseLayers = {
    ...
    'Voyager / Streets': baseVoyager,
    'Outdoor (детальна)': baseCyclOSM,
    'Топографічна': baseTopo,
    'Гуманітарна': baseHOT,
    'Темна MapTiler': baseDark,
    '🛰 Супутник (детальний)': baseSatellite,
    '🛰 Свіжий Sentinel-2': baseSentinelFresh,
  };
  const layerKeyMap = {
    osm:baseLight, voyager:baseVoyager, cyclosm:baseCyclOSM, outdoor:baseCyclOSM,
    topo:baseTopo, hot:baseHOT, dark:baseDark, satellite:baseSatellite, sentinel2:baseSentinelFresh
  };
  if(baseMapTiler){
  ...
  function isDarkBaseLayer(layer){
    return layer===baseDark || layer===baseSatellite || layer===baseSentinelFresh;
  }
  ...
  let savedLayer=localStorage.getItem('pvls_map_layer');
  // Migrate names used by the previous satellite experiments.
  if(['satellite_ua','satellite_hybrid','hybrid','fresh_satellite'].includes(savedLayer)) savedLayer='satellite';
