document.addEventListener("DOMContentLoaded", ()=>{
  ...
  // Satellite labels/hybrid overlay intentionally removed.
  // The satellite option below is a plain imagery layer with no labels.

  // Other MapTiler basemaps stay unchanged; satellite is plain Esri World Imagery.
  ...
  const baseSatelliteFresh = L.tileLayer('https://services.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', {
    minZoom:1,
    maxZoom:20,
    crossOrigin:true,
    attribution:'Tiles &copy; Esri — Esri, Vantor, Earthstar Geographics, and the GIS User Community'
  });
  ...
  const baseLayers = {
    ...
    '🛰 Супутник (свіжі знімки)': baseSatelliteFresh,
  };
  ...
  const layerKeyMap = {
    ...
    satellite_ua:baseSatelliteFresh, satellite_hybrid:baseSatelliteFresh, satellite:baseSatelliteFresh, satellite_fresh:baseSatelliteFresh
  };
  ...
  const isDarkBaseLayer = (layer)=> layer===baseDark || layer===baseSatelliteFresh;
