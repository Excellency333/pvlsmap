document.addEventListener("DOMContentLoaded", ()=>{
  ...
  function configurePvlsSatelliteUaOverlay(layer){
    try{
      const mt = layer && layer.getMaptilerMap ? layer.getMaptilerMap() : null;
      if(!mt || mt.__pvlsSatelliteUaOverlay) return;
      mt.__pvlsSatelliteUaOverlay = true;
      const style = mt.getStyle ? mt.getStyle() : null;
      const layers = style && Array.isArray(style.layers) ? style.layers : [];
      for(const sl of layers){
        ...
        try{
          if(sl.type==='background'){
            mt.setPaintProperty(id,'background-opacity',0);
            continue;
          }
          if(['fill','fill-extrusion','raster','hillshade','heatmap','circle'].includes(sl.type)){
            mt.setLayoutProperty(id,'visibility','none');
            continue;
          }
          if(sl.type==='line'){
            const keep=/(road|street|transport|motorway|trunk|primary|secondary|tertiary|boundary|admin|rail)/i.test(id);
            if(!keep){ mt.setLayoutProperty(id,'visibility','none'); continue; }
            try{ mt.setPaintProperty(id,'line-color','rgba(255,255,255,0.78)'); }catch(_){ }
            try{ mt.setPaintProperty(id,'line-opacity',0.78); }catch(_){ }
            continue;
          }
          if(sl.type==='symbol'){
            const hasText=sl.layout && sl.layout['text-field']!=null;
            const keepText=hasText && /(place|settlement|city|town|village|hamlet|locality|road|street|transport|route|admin|country|state|region)/i.test(id);
            if(!keepText){ mt.setLayoutProperty(id,'visibility','none'); continue; }
            try{ mt.setPaintProperty(id,'text-color','#ffffff'); }catch(_){ }
            try{ mt.setPaintProperty(id,'text-halo-color','rgba(0,0,0,0.92)'); }catch(_){ }
            try{ mt.setPaintProperty(id,'text-halo-width',2.2); }catch(_){ }
            try{ mt.setPaintProperty(id,'text-halo-blur',0.45); }catch(_){ }
            try{ mt.setPaintProperty(id,'text-opacity',1); }catch(_){ }
            try{ mt.setPaintProperty(id,'icon-opacity',0); }catch(_){ }
            const oldSize=sl.layout && sl.layout['text-size'];
            if(oldSize!=null && /(place|settlement|city|town|village|hamlet|locality)/i.test(id)){
              try{
                if(typeof oldSize==='number') mt.setLayoutProperty(id,'text-size',oldSize*1.14);
                else if(Array.isArray(oldSize)) mt.setLayoutProperty(id,'text-size',['*',oldSize,1.14]);
              }catch(_){ }
            }
          }
        }catch(_){ }
      ...
      console.warn('Satellite UA overlay styling skipped',err);
    }
  }

  function createPvlsSatelliteUaVectorOverlay(){
    const key=String(window.PVLS_MAPTILER_KEY||'').trim();
    if(!key) return null;
    try{
      const factory=
      ...
      if(layer && typeof layer.on==='function'){
        layer.on('ready',()=>{
          try{ if(typeof layer.setLanguage==='function') layer.setLanguage('uk'); }catch(_){ }
          configurePvlsSatelliteUaOverlay(layer);
        });
      }
      return layer || null;
    }catch(err){
      console.warn('Satellite UA vector overlay unavailable',err);
      return null;
    }
  }

  function createPvlsSatelliteUaFallbackLabels(){
    const defs=[
      ['Павлоград',48.535,35.870,9,20],['Тернівка',48.523,36.092,9,17],['Шахтарське',48.390,36.200,9,17],
      ['Петропавлівка',48.456,36.438,9,17],['Юр’ївка',48.816,35.918,9,16],['Вербки',48.615,36.050,10,14],
      ['Богданівка',48.485,36.020,10,14],['Богуслав',48.448,35.710,10,14],['Межиріч',48.592,35.758,10,14],
      ['Самарське',48.420,35.975,10,14],['Олефірівка',48.365,35.927,10,13],['Дмитрівка',48.309,36.221,10,13],
      ['Мар’ївка',48.330,36.113,10,13],['Синельникове',48.321,35.517,9,16],['Дніпро',48.464,35.046,8,21]
    ];
    const g=L.layerGroup();
    const items=[];
    for(const [text,lat,lng,minZoom,size] of defs){
      const icon=L.divIcon({
        className:'',
        iconSize:null,
        iconAnchor:[0,0],
        html:`<div style="transform:translate(-50%,-50%);white-space:nowrap;font-family:Arial,sans-serif;font-size:${size}px;font-weight:900;color:#fff;text-shadow:-1px -1px 0 #111,1px -1px 0 #111,-1px 1px 0 #111,1px 1px 0 #111,0 0 6px rgba(0,0,0,.85);pointer-events:none">${text}</div>`
      });
      items.push({minZoom,marker:L.marker([lat,lng],{icon,interactive:false,keyboard:false})});
    }
    function refresh(){
      const z=map.getZoom();
      for(const item of items){
        const has=g.hasLayer(item.marker);
        if(z>=item.minZoom && !has) g.addLayer(item.marker);
        else if(z<item.minZoom && has) g.removeLayer(item.marker);
      }
    }
  ...
  function createPvlsSatelliteUaLayer(){
    const imagery=L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',{
      minZoom:1,maxZoom:20,crossOrigin:true,
      attribution:'Tiles &copy; Esri — Source: Esri, Maxar, Earthstar Geographics, and the GIS User Community'
    });
    const vectorOverlay=createPvlsSatelliteUaVectorOverlay();
    const labels=vectorOverlay || createPvlsSatelliteUaFallbackLabels();
  ...
  const baseMapTiler = createPvlsMapTilerStyle('streets-v4', {language:'uk', enhanceLabels:true});
  const baseSatelliteUa = createPvlsSatelliteUaLayer();
