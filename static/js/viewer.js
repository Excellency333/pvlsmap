document.addEventListener("DOMContentLoaded", ()=>{
  ...
  const layerKeyMap = {osm:baseLight,voyager:baseVoyager,cyclosm:baseCyclOSM,topo:baseTopo,hot:baseHOT,dark:baseDark};
  if(baseMapTiler){
    baseLayers['MapTiler 🇺🇦 (чіткі назви)'] = baseMapTiler;
    layerKeyMap.maptiler = baseMapTiler;
  }
  const allBaseLayers = Object.values(layerKeyMap);
  ...
  function setBaseLayer(layer){
    ...
    applyVisualTheme(currentBase===baseDark ? 'dark' : 'light');
    updatePvlsPlaceLabels();
  }

  function applyTheme(theme){
    setBaseLayer(theme==="dark" ? baseDark : baseLight);
  }
  ...
  if(themeBtn){
    themeBtn.addEventListener("click", ()=>{
      applyTheme(currentBase===baseDark ? "light" : "dark");
    });
  }
